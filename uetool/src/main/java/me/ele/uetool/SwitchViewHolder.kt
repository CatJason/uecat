...
class SwitchViewHolder(itemView: View, private val callback: AttrDialogCallback?) : BaseViewHolder<SwitchItem>(itemView) {
            ...
            try {
                when (item?.type) {
                    ...
                    SwitchItem.Type.TYPE_SHOW_VALID_VIEWS -> {
                        ...
                        val finalItem = item
                        if(finalItem != null) {
                            val view = finalItem.element.view
                            val textView = view as? TextView
                            if(textView != null) {
                                if (finalItem.type == SwitchItem.Type.TYPE_IS_BOLD) {
